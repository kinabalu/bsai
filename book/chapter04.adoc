...
Here are the steps we're going to go through to have a working voice assistant.

1. Pass the command text we want to the `TextToSpeechService` to generate an audio file with speech that can be later processed.
2. Take the audio file generated and pass it to the `TranscribeService` for the prompt.
3. Next we'll pass the text in a `Prompt` and ask the LLM for an answer.
4. We'll pass the text of the answer received back into the `TextToSpeechService` and save it to disk.




we'll use the transcription to understand what is said, and then pass it to an LLM to have it do a thing, and then pass the text from the LLM back in
