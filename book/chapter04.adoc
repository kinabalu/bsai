...
////
TODO I think I'd like to see a preface talking about how AI can be used here."One can imagine
using AI to transcribe a recording to text" and "... or it could be used in an assistive
capacity to read text aloud for the vision-impaired," for example. That sets the stage for what the chapter
describes.
////
...
If you'd like to download the file using the command line tool `curl` you can see how to do that below, or opening a browser and saving the file to disk from the URL: `https://upload.wikimedia.org/wikipedia/commons/8/8f/Daisy_Bell_sung_by_DECtalk.flac`.

////
TODO: bit.ly? Any other url shortener? These long urls are brutal.
////
