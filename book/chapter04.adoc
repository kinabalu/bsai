...
Our next task is to modify our `@Configuration` to include the `UpdateChatService` along with it's dependencies and some mocked light status that we can act on with our voice assistant. The block below can be included anywhere within the `Ch04Configuration` file:
