...
_Now_ we have a pattern that we can use to throw at an LLM, with different `temperature` and `top_p` values, and measure those parameters' effect on the resultsfootnote:[And you readers probably thought we just wanted to type "Jaccard" out a lot, to show off, didn't you?].
