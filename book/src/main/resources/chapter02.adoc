...
[source,java]
----
include::../../../../code/chapter02/src/test/java/ch02/Ch02FirstTests.java[]
----
...
[source,java]
----
include::../../../../code/chapter02/src/test/java/ch02/Ch02OptionTests.java[]
----
...
_Specifying_ either one is trivial: in the code where we build the `OpenAiChatOptions`, simply include a `temperature` value:
...
Well, let's follow the process we've used so far, and show it with some tests.
...
We have two sets of data: one set is a comparison of identical texts, that should have a _very high similarity_, and another set of texts (that we typed very quickly, thank you very much) that have relatively little similarity.

The similarity of `1.0` means those texts are likely to be identical (as, in fact, they are, but similarity scores _usually_ trim out incidental words and punctuation, which we're not doing here).

The similarity score of `0.04` means that the texts are very different, as we expect, since the number of common words is very low.

_Now_ we have a pattern that we can use to throw at an LLM, with different temperatures and `top_k` values, and measure those paramaters' effect on the resultsfootnote:[And you readers probably thought we just wanted to type "Jaccard" out a lot, to show off, didn't you?].
