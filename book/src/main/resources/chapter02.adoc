...
Thankfully, this is entirely supported by `Prompt`, which has a constructor that accepts `List<Message>`; our new `ConversationChatService` will do little more than handle the plumbing of constructing the actual `Prompt` and returning the response.
...
The first part of `interactiveConversation()` replicates `simpleConversation()`; the main difference is that it constructs a `List<Message>` and uses that with the `ConversationChatService`, instead of constructing an immutable `List` in place.
...
You can develop this even further if you like.
Imagine adding `What's the difference in the slope?` as part of the conversation as well, and you'll see the LLM use even more of its prior answer for context.
