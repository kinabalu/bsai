...
Here, though, we're going to use inheritance, because the only difference between `RequestChatService` and `UpdateChatService` is the provision of the `UpdateLightStatusFunction` function to the chat options.
