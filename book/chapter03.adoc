...
So in essence, our `ChangeLightStatusService` is going to be _just_ like our `RequestLightStatusService`, with a mutation of the light and a `Request` that includes the desired light's state. Let's see what it looks like:
...
It's so similar to `RequestLightStatusService` that one could conceive of writing a class hierarchy to build the functionality - you'd use a generic type for the `Request`, and have localized functionality to apply the request (i.e., a do-nothing operation for `RequestLightStatusService` and a mutation operation for `ChangeLightStatusService`). It'd be trivial to write, really, but wouldn't really accomplish anything for our purposes here, besides creating a few more listings. In the end, it'd probably end up with _more_ lines of code, by a few, thanks to Java being slightly verbosefootnote:[If we put the functions in our Spring Configuration as `@Bean` instances rather than in separate source files, we could probably save a few lines here and there, but then it'd get a lot harder to explain in this chapter.].
...
Here, though, we're going to use inheritance, because the only difference between `LightQueryService` and `LightUpdateService` is the provision of the `ChangeLightStatusService` function to the chat options.
...
[source,text]
----
INFO  ch03.service.ChangeLightStatusService --
...
INFO  ch03.service.RequestLightStatusService --
Requesting status for light: Request[color=yellow]
INFO  ch03.ChangeLightStatusTest --
...
You can see the flow: first, it determined that it needed to mutate the `yellow` light, so called `ChangeLightStatusService` with `yellow` and `true` to turn the light "on" - and then it iterated through the lights we specified ("red", "green", "blue", "purple", and "yellow") to query their state, and it informed us that `purple` wasn't available, as the function to get the light returned `null`.
...
That's... one way to do it. We can actually instruct Spring AI to do the work for us, though. Let's build a test that does the same thing as `ChangeLightStatusTest`, except that it returns a list of lights - and adds the CIE 1931 color coordinate of the light, inferred from its state and name.
...
Of course, now that we've talked about it, we should show it working, with a test. Here's `ChangeLightStructuredTest`.
